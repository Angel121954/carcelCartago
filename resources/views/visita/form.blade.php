<div class="space-y-6">
    ...
    <div>
        ...
        <x-text-input id="hora_inicio" name="hora_inicio" type="time" class="mt-1 block w-full" :value="old('hora_inicio', $visita?->hora_inicio)" autocomplete="hora_inicio" placeholder="Hora Inicio" />
        <x-input-error class="mt-2" :messages="$errors->get('hora_inicio')" />
    </div>
    <div>
        <x-input-label for="hora_fin" :value="__('Hora Fin')" />
        <x-text-input id="hora_fin" name="hora_fin" type="time" class="mt-1 block w-full" :value="old('hora_fin', $visita?->hora_fin)" autocomplete="hora_fin" placeholder="Hora Fin" />
